...
def test_build_stage_copies_installer_scripts_and_writes_model_asset_metadata(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(module, "WIZARD_IMAGE", dist_dir / "windows-installer" / "ai-safe-plugin-wizard.bmp")
    monkeypatch.setattr(module, "WIZARD_SMALL_IMAGE", dist_dir / "windows-installer" / "ai-safe-plugin-wizard-small.bmp")
    ...
    # Wizard BMPs are generated by build_stage and must be valid 24-bit BMPs.
    assert module.WIZARD_IMAGE.exists()
    assert module.WIZARD_IMAGE.read_bytes()[:2] == b"BM"
    assert module.WIZARD_SMALL_IMAGE.exists()
    assert module.WIZARD_SMALL_IMAGE.read_bytes()[:2] == b"BM"
...
def test_inno_setup_script_uses_branding_extension_id_capture_and_model_download_pages():
    ...
    assert "WizardImageFile=..\\..\\dist\\windows-installer\\ai-safe-plugin-wizard.bmp" in script
    assert "WizardSmallImageFile=..\\..\\dist\\windows-installer\\ai-safe-plugin-wizard-small.bmp" in script
