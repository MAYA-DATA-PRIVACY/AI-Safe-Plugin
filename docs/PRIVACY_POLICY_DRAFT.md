...
- Replace the old "Maya File Anonymization Extension" scope. The current product is Veil, a browser extension plus local GLiNER2 server for AI/chat text inputs, not a file upload anonymization product.
...
- Avoid saying Veil collects analytics, beta improvement data, or performance monitoring unless those systems are actually implemented and disclosed. I found local settings, local stats, release checks, local logs, and optional Maya anonymization, but no general telemetry pipeline.
- Disclose local browser storage clearly. Veil stores settings, custom patterns, API keys, HF token if provided, local server URL override, onboarding state, site counters, alias ledgers, and cached redaction state in `chrome.storage.local`.
...
- Do not imply saved secrets are encrypted by Veil. Maya API keys and HF tokens are stored in browser-local extension storage.
...
- Consider tightening local server CORS/auth before claiming only the Veil extension can call local detection endpoints.
...
Users can remove browser-local data by removing the extension or clearing extension storage. Users can remove the local server runtime, model files, autostart entries, and native messaging host configuration by running the Veil uninstall scripts.
